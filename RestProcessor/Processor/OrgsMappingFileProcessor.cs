namespace RestProcessor
{
    ...
    public class OrgsMappingProcessor : MappingProcessorBase
    {
        ...
        {
            var targetApiDir = GetApiDirectory(targetRootDir, orgsMappingFile.TargetApiRootDir);
            var targetIndexPath = Path.Combine(targetApiDir, Constants.AutoGeneratedPage.TargetFileName);
            ...
            using (var writer = new StreamWriter(targetIndexPath))
            {
                ...
                writer.WriteLine("# REST APIs at Microsoft");
                writer.WriteLine();

                var summaryFile = Path.Combine(targetRootDir, Constants.AutoGeneratedPage.SummaryFileName);
                ...
                foreach (var orgInfo in orgsMappingFile.OrgInfos)
                {
                    ...
                    if (orgInfo.Services.Count > 0)
                    {
                        ...
                        foreach (var service in orgInfo.Services)
                        {
                            ...
                            var summary = Utility.GetYamlHeaderByMeta(Path.Combine(targetRootDir, service.IndexFile), Constants.AutoGeneratedPage.DescriptionMetaName);
